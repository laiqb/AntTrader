...
use ant_common::{cache::CacheConfig, enums::Environment, logging::logger::LoggerConfig};
use ant_core::UUID4;
use ant_data::engine::config::DataEngineConfig;
use ant_execution::engine::config::ExecutionEngineConfig;
use ant_model::identifiers::TraderId;
use ant_portfolio::config::PortfolioConfig;
use ant_risk::engine::config::RiskEngineConfig;

use crate::{config::KernelConfig, kernel::AntKernel};

/// Builder for constructing a [`AntKernel`] with a fluent API.
///
/// Provides a convenient way to configure and build a kernel instance with
/// optional components and settings.
#[derive(Debug)]
pub struct AntKernelBuilder {
    name: String,
    trader_id: TraderId,
...
impl AntKernelBuilder {
    /// Creates a new [`AntKernelBuilder`] with required parameters.
    #[must_use]
    ...
    /// Build the [`AntKernel`] with the configured settings.
    ///
    /// # Errors
    ///
    /// Returns an error if kernel initialization fails.
    pub fn build(self) -> anyhow::Result<AntKernel> {
        ...
        AntKernel::new(self.name, config)
    }
}

impl Default for AntKernelBuilder {
...
mod tests {
    ...
    fn test_builder_default() {
        let builder = AntKernelBuilder::default();
        assert_eq!(builder.name, "AntKernel");
    ...
    fn test_builder_fluent_api() {
        ...
        let builder =
            AntKernelBuilder::new("TestKernel".to_string(), trader_id, Environment::Live)
    ...
    fn test_builder_build() {
        ...
        let result = AntKernelBuilder::default().build();
        assert!(result.is_ok());

        let kernel = result.unwrap();
        assert_eq!(kernel.name(), "AntKernel".to_string());
