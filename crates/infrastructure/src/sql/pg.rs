...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.infrastructure")
)]
...
/// Scans the current working directory for the `ant_trader` repository
...
fn get_schema_dir() -> anyhow::Result<String> {
    std::env::var("SCHEMA_DIR").or_else(|_| {
        let ant_git_repo_name = "ant_trader";
        let binding = std::env::current_dir().unwrap();
        let current_dir = binding.to_str().unwrap();
        match current_dir.find(ant_git_repo_name){
            Some(index) => {
                let schema_path = current_dir[0..index + ant_git_repo_name.len()].to_string() + "/schema/sql";
                Ok(schema_path)
            }
