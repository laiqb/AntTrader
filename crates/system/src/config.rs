...
use ant_persistence::config::StreamingConfig;
use ant_portfolio::config::PortfolioConfig;
use ant_risk::engine::config::RiskEngineConfig;

/// Configuration trait for a `AntKernel` core system instance.
pub trait AntKernelConfig: Debug {
...
/// Basic implementation of `AntKernelConfig` for builder and testing.
#[derive(Debug, Clone)]
...
impl AntKernelConfig for KernelConfig {
    fn environment(&self) -> Environment {
        self.environment
    }
