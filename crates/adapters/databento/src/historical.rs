...
use ant_core::{UnixNanos, consts::ANT_USER_AGENT, time::AtomicTime};
use ant_model::{
...
use crate::{
    ...
    symbology::{
        MetadataCache, check_consistent_symbology, decode_ant_instrument_id,
...
impl DatabentoHistoricalClient {
    ...
    pub fn new(
        ...
        let client = databento::HistoricalClient::builder()
            .user_agent_extension(ANT_USER_AGENT.into())
            .key(key.clone())
    ...
    pub async fn get_range_instruments(
        ...
        while let Ok(Some(msg)) = decoder.decode_record::<dbn::InstrumentDefMsg>().await {
            ...
            let mut instrument_id = decode_ant_instrument_id(
                &record,
                &mut metadata_cache,
    ...
    pub async fn get_range_quotes(
        ...
        let mut process_record = |record: dbn::RecordRef| -> anyhow::Result<()> {
            let sym_map = self
                ...
                .read()
                .map_err(|e| anyhow::anyhow!("symbol_venue_map lock poisoned: {e}"))?;
            let instrument_id = decode_ant_instrument_id(
                &record,
                &mut metadata_cache,
                &self.publisher_venue_map,
                &sym_map,
