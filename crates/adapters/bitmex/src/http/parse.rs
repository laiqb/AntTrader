...
use ant_core::{UnixNanos, time::get_atomic_clock_realtime, uuid::UUID4};
use ant_model::{
...
/// Parse a BitMEX index instrument into a ant `InstrumentAny`.
///
...
/// Parse a BitMEX spot instrument into a ant `InstrumentAny`.
///
/// # Errors
///
/// Returns an error if values are out of valid range or cannot be parsed.
...
/// Parse a BitMEX order into a ant `OrderStatusReport`.
///
/// # Errors
///
/// Currently this function does not return errors as all fields are handled gracefully,
...
pub fn parse_order_status_report(
    ...
    .into();
    let time_in_force: ant_model::enums::TimeInForce = (*order
        .time_in_force
        .as_ref()
...
/// Parse a BitMEX execution into a ant `FillReport`.
///
/// # Errors
///
/// Currently this function does not return errors as all fields are handled gracefully,
...
/// Parse a BitMEX position into a ant `PositionStatusReport`.
///
/// # Errors
///
/// Currently this function does not return errors as all fields are handled gracefully,
...
mod tests {
    ...
    use ant_model::enums::{LiquiditySide, PositionSide};
    use rstest::rstest;
