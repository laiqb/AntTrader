...
use ant_core::{
    UnixNanos, consts::ANT_USER_AGENT, env::get_env_var, time::get_atomic_clock_realtime,
};
use ant_model::{
...
use ant_network::{http::HttpClient, ratelimiter::quota::Quota};
...
impl CoinbaseIntxHttpInnerClient {
    ...
    fn default_headers() -> HashMap<String, String> {
        HashMap::from([(USER_AGENT.to_string(), ANT_USER_AGENT.to_string())])
    }
...
impl CoinbaseIntxHttpClient {
    ...
    pub async fn request_order_status_report(
        ...
        let report = parse_order_status_report(
            resp,
            account_id,
            0, 0, // TODO laiqb check instrument.price_precision(), instrument.size_precision(),
            ts_init,
        )?;
        Ok(report)
    }
    ...
    pub async fn request_position_status_report(
        ...
        let report =
            parse_position_status_report(resp, account_id,
                                        0, // TODO laiqb check instrument.size_precision(),
    ...
    pub async fn submit_order(
        ...
        let report = parse_order_status_report(
            resp,
            account_id,
            0, // TODO laiqb check  instrument.price_precision(),
            0, // TODO laiqb check  instrument.size_precision(),
            ts_init,
        )?;
        Ok(report)
