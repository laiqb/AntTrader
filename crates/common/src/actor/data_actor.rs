    ...
    Component + Deref<Target = DataActorCore> + DerefMut<Target = DataActorCore>
{
    ...
    fn handle_data(&mut self, data: &dyn Any) {
        ...
        if let Err(e) = self.on_data(data) { //触发数据处理
            log_error(&e);
        }
    }
    ...
    {
        let actor_id = self.actor_id().inner(); //actorId
        let handler = ShareableMessageHandler(Rc::new(TypedMessageHandler::with_any(
            move |data: &dyn Any| {
                get_actor_unchecked::<Self>(&actor_id) // 从ActorRegistry 中 Get Actor mut type
                    .handle_data(data); // 调用具体实例 handle 处理数据
            },
        ))); //数据处理的handler
        // client_id 可为 None
    ...
    T: DataActor + Debug + 'static,
{
    ...
    fn register(
        ...
        clock.borrow_mut().register_default_handler(callback); // 设置默认 时间handler

        self.initialize() //组件初始化
    }
...
impl DataActorCore {
    ...
    fn add_subscription(&mut self, topic: MStr<Topic>, handler: ShareableMessageHandler) {
        ...
        self.topic_handlers.insert(topic, handler.clone()); // DataActorCore sava handler 将 topic 与 handler 保存到 topic_handler
        msgbus::subscribe_topic(topic, handler, None); // msgbus handler save msgbus保存
    }
    ...
    pub fn subscribe_data(
        ...
        let topic = get_custom_topic(&data_type); //与 message_bus 产生交互
