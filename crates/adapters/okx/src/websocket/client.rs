...
//! and parses incoming messages into structured ant domain objects.
...
use ant_common::runtime::get_runtime;
use ant_core::{
    UUID4, consts::ANT_USER_AGENT, env::get_env_var, time::get_atomic_clock_realtime,
};
use ant_model::{
    data::BarType,
...
use super::{
    ...
    error::OKXWsError,
    messages::{
        ExecutionReport, AntWsMessage, OKXAuthentication, OKXAuthenticationArg,
...
use crate::{
    common::{
        consts::{
            OKX_ANT_BROKER_ID, OKX_SUPPORTED_ORDER_TYPES, OKX_SUPPORTED_TIME_IN_FORCE,
            OKX_WS_PUBLIC_URL,
        },
...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.adapters")
)]
pub struct OKXWebSocketClient {
    ...
    rx: Option<Arc<tokio::sync::mpsc::UnboundedReceiver<AntWsMessage>>>,
...
impl OKXWebSocketClient {
    ...
    pub async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        let config = WebSocketConfig {
            url: self.url.clone(),
            headers: vec![(USER_AGENT.to_string(), ANT_USER_AGENT.to_string())],
        ...
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel::<AntWsMessage>();
    ...
    pub fn stream(&mut self) -> impl Stream<Item = AntWsMessage> + 'static {
        let rx = self
            .rx
            .take()
    ...
    /// Modifies multiple orders via WebSocket using ant domain types.
    #[allow(clippy::type_complexity)]
    #[allow(clippy::too_many_arguments)]
...
impl OKXWsMessageHandler {
    ...
    async fn next(&mut self) -> Option<AntWsMessage> {
        ...
        while let Some(event) = self.handler.next().await {
            ...
            if let OKXWebSocketEvent::Login { code, msg, .. } = event {
                ...
                continue; // Don't forward login events as ant messages
            }
            ...
            if let OKXWebSocketEvent::BookData { arg, action, data } = event {
                ...
                match parse_book_msg_vec(
                    ...
                    Ok(data) => return Some(AntWsMessage::Data(data)),
                    Err(e) => {
                        ...
                        match op {
                            OKXWsOperation::Order => {
                                ...
                                )) = self.pending_place_requests.remove(id)
                                {
                                    ...
                                    return Some(AntWsMessage::OrderRejected(rejected));
                                }
                            }
                            OKXWsOperation::CancelOrder => {
                                ...
                                )) = self.pending_cancel_requests.remove(id)
                                {
                                    ...
                                    return Some(AntWsMessage::OrderCancelRejected(rejected));
                                }
                            }
                            OKXWsOperation::AmendOrder => {
                                ...
                                )) = self.pending_amend_requests.remove(id)
                                {
                                    ...
                                    return Some(AntWsMessage::OrderModifyRejected(rejected));
                                }
                            }
                            OKXWsOperation::MassCancel => {
                                    ...
                                    self.pending_mass_cancel_requests.remove(id)
                                {
                                    ...
                                    return Some(AntWsMessage::Error(error));
                                } else {
                                    tracing::error!(
            ...
            if let OKXWebSocketEvent::Data { ref arg, ref data } = event {
                            ...
                            if let Some(account) = accounts.first() {
                                ...
                                match parse_account_state(account, self.account_id, ts_init) {
                                    Ok(account_state) => {
                                        ...
                                        return Some(AntWsMessage::AccountUpdate(
                                            account_state,
                                        ));
                                    }
                                    Err(e) => {
                ...
                if arg.channel == OKXWsChannel::Orders {
                    ...
                    if !exec_reports.is_empty() {
                        return Some(AntWsMessage::ExecutionReports(exec_reports));
                    }
                }
            ...
            if matches!(&event, OKXWebSocketEvent::Reconnected) {
                return Some(AntWsMessage::Reconnected);
            }
        }
...
mod tests {
    ...
    fn test_websocket_error_handling() {
        ...
        let ant_msg = AntWsMessage::Error(error);
        match ant_msg {
            AntWsMessage::Error(err) => {
