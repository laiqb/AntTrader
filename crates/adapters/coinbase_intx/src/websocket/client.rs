...
use ant_common::{logging::log_task_stopped, runtime::get_runtime};
use ant_core::{
    consts::ANT_USER_AGENT, env::get_env_var, time::get_atomic_clock_realtime,
};
use ant_model::{
...
use super::{
    ...
    messages::{CoinbaseIntxSubscription, CoinbaseIntxWsMessage, AntWsMessage},
    parse::{
    ...
    pub fn is_active(&self) -> bool {
        self.inner
            ...
            .and_then(|guard| {
                guard
                    .as_ref()
                    .map(ant_network::websocket::WebSocketClient::is_active)
            })
            .unwrap_or(false)
    }
    ...
    pub fn is_closed(&self) -> bool {
        self.inner
            ...
            .and_then(|guard| {
                guard
                    .as_ref()
                    .map(ant_network::websocket::WebSocketClient::is_closed)
            })
            .unwrap_or(true)
    }
...
/// Provides a ant parser for the Coinbase International WebSocket feed.
            ...
            match event {
                CoinbaseIntxWsMessage::Instrument(msg) => {
                    if let Some(inst) = parse_instrument_any(&msg, clock.get_time_ns()) {
                        ...
                        return Some(AntWsMessage::Instrument(inst));
                    }
                }
                    ...
                    if let Some(inst) = self.instruments_cache.get(&msg.product_id) {
                        match parse_orderbook_snapshot_msg(
                            ...
                            Ok(deltas) => {
                                ...
                                return Some(AntWsMessage::Data(data));
                            }
                            Err(e) => {
                ...
                CoinbaseIntxWsMessage::Quote(msg) => {
                    if let Some(inst) = self.instruments_cache.get(&msg.product_id) {
                        match parse_quote_msg(
                            ...
                            Ok(quote) => return Some(AntWsMessage::Data(Data::Quote(quote))),
                            Err(e) => {
                ...
                CoinbaseIntxWsMessage::Trade(msg) => {
                    if let Some(inst) = self.instruments_cache.get(&msg.product_id) {
                        match parse_trade_msg(
                            ...
                            Ok(trade) => return Some(AntWsMessage::Data(Data::Trade(trade))),
                            Err(e) => {
                    ...
                    if let Some(inst) = self.instruments_cache.get(&msg.product_id) {
                        ...
                        match (mark_price, index_price) {
                            (Some(mark), Some(index)) => {
                                return Some(AntWsMessage::MarkAndIndex((mark, index)));
                            }
                            (Some(mark), None) => return Some(AntWsMessage::MarkPrice(mark)),
                            (None, Some(index)) => {
                                return Some(AntWsMessage::IndexPrice(index));
                            }
                            (None, None) => continue,
                        };
                    }
                ...
                CoinbaseIntxWsMessage::CandleSnapshot(msg) => {
                    if let Some(inst) = self.instruments_cache.get(&msg.product_id) {
                        match parse_candle_msg(
                            ...
                            Ok(bar) => return Some(AntWsMessage::Data(Data::Bar(bar))),
                            Err(e) => {
