...
use crate::websocket::{BitmexWebSocketClient, messages::AntWsMessage};
            ...
            tokio::spawn(async move {
                ...
                while let Some(msg) = stream.next().await {
                    Python::with_gil(|py| match msg {
                        ...
                        AntWsMessage::FillReports(reports) => {
                            for report in reports {
                                if let Ok(py_obj) = report.into_py_any(py) {
                                    call_python(py, &callback, py_obj);
                                }
                        ...
                        AntWsMessage::FundingRateUpdates(updates) => {
                        ...
                        AntWsMessage::OrderUpdated(event) => {
