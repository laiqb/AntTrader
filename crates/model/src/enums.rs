...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(eq, eq_int, module = "ant_trader.core.ant_pyo3.model.enums")
)]
...
pub enum AggregationSource {
    /// The data is externally aggregated (outside the ant system boundary).
    External = 1,
    /// The data is internally aggregated (inside the ant system boundary).
    Internal = 2,
}
...
pub enum OrderStatus {
    /// The order is initialized (instantiated) within the ant system.
    Initialized = 1,
    /// The order was denied by the ant system, either for being invalid, unprocessable or exceeding a risk limit.
    Denied = 2,
    /// The order became emulated by the ant system in the `OrderEmulator` component.
    Emulated = 3,
    /// The order was released by the ant system from the `OrderEmulator` component.
    Released = 4,
    /// The order was submitted by the ant system to the external service or trading venue (awaiting acknowledgement).
    Submitted = 5,
