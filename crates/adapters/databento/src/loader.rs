...
use super::{
    ...
    symbology::decode_ant_instrument_id,
...
/// A ant data loader for Databento Binary Encoding (DBN) format data.
///
/// # Supported schemas:
///  - `MBO` -> `OrderBookDelta`
...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.databento")
)]
#[derive(Debug)]
            ...
            match dbn_stream.get() {
                Some(rec) => {
                    ...
                    let instrument_id = match &instrument_id {
                        Some(id) => *id, // Copy
                        None => match decode_ant_instrument_id(
                            &record,
                            &mut metadata_cache,
                            &self.publisher_venue_map,
                            &self.symbol_venue_map,
...
mod tests {
    ...
    use ant_model::types::{Price, Quantity};
