...
use ant_data::engine::DataEngine;
use ant_execution::engine::ExecutionEngine;
use ant_model::identifiers::TraderId;
use ant_portfolio::portfolio::Portfolio;
use ant_risk::engine::RiskEngine;
use ustr::Ustr;

use crate::{builder::AntKernelBuilder, config::AntKernelConfig, trader::Trader};

/// Core Ant system kernel.
///
/// Orchestrates data and execution engines, cache, clock, and messaging across environments.
#[derive(Debug)]
pub struct AntKernel {
    ...
    pub config: Box<dyn AntKernelConfig>,
...
impl AntKernel {
    /// Create a new [`AntKernelBuilder`] for fluent configuration.
    #[must_use]
    pub const fn builder(
        name: String,
        trader_id: TraderId,
        environment: ant_common::enums::Environment,
    ) -> AntKernelBuilder {
        AntKernelBuilder::new(name, trader_id, environment)
    }

    /// Create a new [`AntKernel`] instance.
    ///
    /// # Errors
    ///
    /// Returns an error if the kernel fails to initialize.
    pub fn new<T: AntKernelConfig + 'static>(name: String, config: T) -> anyhow::Result<Self> {
    ...
    /// Resets the Ant system kernel to its initial state.
    ...
    /// Disposes of the Ant system kernel, releasing resources.
