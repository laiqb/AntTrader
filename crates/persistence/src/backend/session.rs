...
use ant_core::{UnixNanos, ffi::cvec::CVec};
use ant_model::data::{Data, HasTsInit};
use ant_serialization::arrow::{
...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.persistence")
)]
...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.persistence", unsendable)
)]
...
impl DataQueryResult {
    ...
    pub fn drop_chunk(&mut self) {
        if let Some(CVec { ptr, len, cap }) = self.chunk.take() {
            let data: Vec<Data> =
                unsafe { Vec::from_raw_parts(ptr.cast::<ant_model::data::Data>(), len, cap) };
            drop(data);
        }
    }
}
