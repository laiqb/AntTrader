...
use ant_core::python::{to_pyruntime_err, to_pyvalue_err};
use ant_model::{
...
use crate::{
    ...
    websocket::{
        OKXWebSocketClient,
        messages::{ExecutionReport, AntWsMessage, OKXWebSocketError},
    },
};
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            tokio::spawn(async move {
                ...
                while let Some(msg) = stream.next().await {
                    match msg {
                        AntWsMessage::Instrument(msg) => {
                            call_python_with_data(&callback, |py| {
                                instrument_any_to_pyobject(py, *msg)
                            });
                        }
                        AntWsMessage::Data(msg) => Python::with_gil(|py| {
                            for data in msg {
                                let py_obj = data_to_pycapsule(py, data);
                                call_python(py, &callback, py_obj);
                            }
                        }),
                        AntWsMessage::FundingRates(msg) => {
                            for data in msg {
                                call_python_with_data(&callback, |py| data.into_py_any(py));
                            }
                        }
                        AntWsMessage::OrderRejected(msg) => {
                            call_python_with_data(&callback, |py| msg.into_py_any(py))
                        }
                        AntWsMessage::OrderCancelRejected(msg) => {
                            call_python_with_data(&callback, |py| msg.into_py_any(py))
                        }
                        AntWsMessage::OrderModifyRejected(msg) => {
                            call_python_with_data(&callback, |py| msg.into_py_any(py))
                        }
                        AntWsMessage::ExecutionReports(msg) => {
                            for report in msg {
                                match report {
                        ...
                        AntWsMessage::Deltas(msg) => Python::with_gil(|py| {
                            let py_obj =
                                data_to_pycapsule(py, Data::Deltas(OrderBookDeltas_API::new(msg)));
                            call_python(py, &callback, py_obj);
                        }),
                        AntWsMessage::AccountUpdate(msg) => {
                            call_python_with_data(&callback, |py| msg.py_to_dict(py));
                        }
                        AntWsMessage::Reconnected => {} // Nothing to handle
                        AntWsMessage::Error(msg) => {
                            call_python_with_data(&callback, |py| msg.into_py_any(py));
                        }
                        AntWsMessage::Raw(msg) => {
