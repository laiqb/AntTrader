...
fn register_data_actor(
    ...
    let sender = SyncDataCommandSender; //设置一个发送
    set_data_cmd_sender(Arc::new(sender)); // 设置到 send 中

    let config = DataActorConfig::default(); // 创建一个数据配置
    // Ensure clean message bus state for this actor's subscriptions
    let bus = get_message_bus(); //创建一个线程独立的MessageBus 容器
    *bus.borrow_mut() = MessageBus::default(); // 设置一个新的MessageBus
    let mut actor = TestDataActor::new(config); // 创建TestDataActor
    actor.register(trader_id, clock, cache).unwrap(); // 对TestDataActor 进行设置 cache clock traderId
...
//     assert_eq!(actor_ref.count, 42);
//     assert_eq!(actor_ref.count, actor_ref2.count);
//     assert_eq!(&actor_ref.count, &actor_ref2.count)
// }
//
// #[rstest]
...
//     trader_id: TraderId,
// ) {
//     let actor_id = register_data_actor(clock, cache, trader_id); // 创建并注册 actorRegistry
//     let actor = get_actor_unchecked::<TestDataActor>(&actor_id); // 从actorRegistry 读取出 actor
//     actor.start()// 启动actor
//         .unwrap();
//
//     let data_type = DataType::new(stringify!(String), None);
//     actor
//         .subscribe_data(data_type.clone(), None, None); // actor 订阅数据
//
//     let topic = get_custom_topic(&data_type);
//     let data = String::from("CustomData-01");
//     msgbus::publish(topic, &data); // 发送对应主体的数据
