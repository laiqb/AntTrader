...
use ant_model::identifiers::{InstrumentId, Symbol, Venue};
...
/// Decodes a Databento record into a ant `InstrumentId`.
///
/// # Errors
///
/// Returns an error if:
...
pub fn decode_ant_instrument_id(
    ...
    let mut instrument_id = get_ant_instrument_id_for_record(record, metadata, *venue)?;
...
/// Gets the ant `InstrumentId` for a Databento record.
///
/// # Errors
///
/// Returns an error if:
...
pub fn get_ant_instrument_id_for_record(
    record: &dbn::RecordRef,
    metadata: &mut MetadataCache,
    venue: Venue,
) -> anyhow::Result<InstrumentId> {
...
mod tests {
    ...
    fn test_instrument_id_to_symbol_string_updates_map() {
        use ant_model::identifiers::Venue;
