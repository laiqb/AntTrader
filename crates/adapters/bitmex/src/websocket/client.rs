...
impl BitmexWebSocketClient {
    ...
    pub fn stream(&mut self) -> impl Stream<Item = AntWsMessage> + use<> {
        let rx = self
            .rx
            .take()
                ...
                BitmexWsMessage::Table(table_msg) => {
                    ...
                    return Some(match table_msg {
                        ...
                        BitmexTableMessage::Quote { mut data, .. } => {
                            ...
                            {
                                AntWsMessage::Data(vec![Data::Quote(quote)])
                            } else {
                                continue;
                            }
                        }
                        ...
                        BitmexTableMessage::Order { data, .. } => {
                                ...
                                match order_data {
                                    ...
                                    OrderData::Update(msg) => {
                                        ...
                                        if let Some(event) = parse_order_update_msg(
                                            ...
                                            return Some(AntWsMessage::OrderUpdated(event));
                                        } else {
                                            tracing::warn!(
                            ...
                            AntWsMessage::OrderStatusReports(reports)
                        }
                        ...
                        BitmexTableMessage::Wallet { data, .. } => {
                            if let Some(wallet_msg) = data.into_iter().next() {
                                ...
                                AntWsMessage::AccountState(account_state)
                            } else {
                                continue;
                            }
                        }
                        ...
                        BitmexTableMessage::Funding { data, .. } => {
                            ...
                            if !funding_updates.is_empty() {
                                AntWsMessage::FundingRateUpdates(funding_updates)
                            } else {
                                continue;
                            }
                        }
