...
use ant_core::{UnixNanos, datetime::NANOSECONDS_IN_MICROSECOND};
use ant_model::{
...
/// Parses a ant instrument ID from the given Tardis `exchange` and `symbol` values.
#[must_use]
...
/// Parses a ant instrument ID with a normalized symbol from the given Tardis `exchange` and `symbol` values.
#[must_use]
...
/// Parses a ant price from the given `value`.
///
...
/// Parses a ant order side from the given Tardis string `value`.
#[must_use]
...
/// Parses a ant aggressor side from the given Tardis string `value`.
#[must_use]
...
/// Parses a ant option kind from the given Tardis enum `value`.
#[must_use]
...
/// Parses a ant book action inferred from the given Tardis values.
#[must_use]
...
/// Parses a ant bar specification from the given Tardis string `value`.
///
...
/// Converts a ant `BarSpecification` to the Tardis trade bar string convention.
///
/// # Panics
///
...
mod tests {
    ...
    use ant_model::enums::AggressorSide;
    use rstest::rstest;
