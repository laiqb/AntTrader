...
use ant_model::data::{Bar, OrderBookDelta, QuoteTick, TradeTick};
use ant_persistence::python::backend::session::antDataType;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match data_type {
        antDataType::QuoteTick => process_data::<QuoteTick>(&file_path)?,
        antDataType::TradeTick => process_data::<TradeTick>(&file_path)?,
        antDataType::Bar => process_data::<Bar>(&file_path)?,
        antDataType::OrderBookDelta => process_data::<OrderBookDelta>(&file_path)?,
