...
fn main() {
    ...
    let ant_version = "1.220.0"; // Hardcode to avoid including pyproject.toml in package

    if let Some(pyproject_version) = try_read_pyproject_version() {
        assert!(
            pyproject_version.starts_with(ant_version),
            "Version mismatch: pyproject.toml={pyproject_version}, hardcoded={ant_version}",
        );
    }

    // Set compile-time environment variables
    println!("cargo:rustc-env=ANT_VERSION={ant_version}");
    println!("cargo:rustc-env=ANT_USER_AGENT=AntTrader/{ant_version}");
    ...
    if env::var("CARGO_FEATURE_FFI").is_ok() {
        ...
        let c_header_path = crate_dir.join("../../ant_trader/option/includes/option.h");
        ...
        let cython_path = crate_dir.join("../../ant_trader/option/rust/option.pxd");
