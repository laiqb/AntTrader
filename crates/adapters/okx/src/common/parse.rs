...
/// Parses a ant instrument ID from the given OKX `symbol` value.
#[must_use]
pub fn parse_instrument_id(symbol: Ustr) -> InstrumentId {
    InstrumentId::new(Symbol::from_ustr_unchecked(symbol), *OKX_VENUE)
}

/// Parses a ant client order ID from the given OKX `clOrdId` value.
#[must_use]
...
/// Parses OKX side to ant aggressor side.
...
/// Parses OKX execution type to ant liquidity side.
...
/// Parses quantity to ant position side.
...
/// Parses an OKX mark price record into a ant [`MarkPriceUpdate`].
///
/// # Errors
///
...
/// Parses an OKX index ticker record into a ant [`IndexPriceUpdate`].
///
/// # Errors
///
...
/// Parses an OKX trade record into a ant [`TradeTick`].
///
/// # Errors
///
...
/// Parses an OKX historical candlestick record into a ant [`Bar`].
///
/// # Errors
///
...
/// Parses an OKX order history record into a ant [`OrderStatusReport`].
...
/// Parses an OKX position into a ant [`PositionStatusReport`].
///
/// # Panics
///
...
pub fn parse_position_status_report(
    ...
    // Convert to absolute quantity (positions are always positive in ant)
...
/// Parses an OKX transaction detail into a ant `FillReport`.
///
/// # Errors
///
...
/// parsing each message, and wrapping results in ant Data enum variants.
...
/// Converts ant bar specification to OKX mark price channel.
...
/// Converts ant bar specification to OKX timeframe string.
...
/// Converts OKX timeframe string to ant bar specification.
...
/// This ensures the BarType uses canonical ant format instead of raw OKX strings.
...
/// Parses an OKX instrument definition into a ant instrument.
...
/// Parses an OKX spot instrument definition into a ant currency pair.
...
/// Parses an OKX swap instrument definition into a ant crypto perpetual.
...
/// Parses an OKX futures instrument definition into a ant crypto future.
...
/// Parses an OKX option instrument definition into a ant option contract.
...
/// Parses an OKX account into a ant account state.
