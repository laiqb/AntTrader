...
RUST_INCLUDES = ["ant_trader/core/includes"]
RUST_LIB_PATHS: list[Path] = [
    CARGO_TARGET_DIR / f"{RUST_LIB_PFX}ant_backtest.{RUST_STATIC_LIB_EXT}",
    CARGO_TARGET_DIR / f"{RUST_LIB_PFX}ant_common.{RUST_STATIC_LIB_EXT}",
    CARGO_TARGET_DIR / f"{RUST_LIB_PFX}ant_core.{RUST_STATIC_LIB_EXT}",
    CARGO_TARGET_DIR / f"{RUST_LIB_PFX}ant_model.{RUST_STATIC_LIB_EXT}",
    CARGO_TARGET_DIR / f"{RUST_LIB_PFX}ant_persistence.{RUST_STATIC_LIB_EXT}",
]
...
def _build_extensions() -> list[Extension]:
    ...
    return [
        ...
        for pyx in itertools.chain(Path("ant_trader").rglob("*.pyx"))
    ]
...
def _copy_rust_dylibs_to_project() -> None:
    ...
    src = Path(CARGO_TARGET_DIR) / f"{RUST_LIB_PFX}ant_pyo3.{RUST_DYLIB_EXT}"
    dst = Path("ant_trader/core") / f"ant_pyo3{ext_suffix}"
    shutil.copyfile(src=src, dst=dst)

    print(f"Copied {src} to {dst}")


def _get_ant_version() -> str:
...
def _strip_unneeded_symbols() -> None:
    try:
        ...
        for so in itertools.chain(Path("ant_trader").rglob("*.so")):
...
if __name__ == "__main__":
    print("\033[36m")
    print("=====================================================================")
    print(f"Ant Builder {_get_ant_version()}")
    print("=====================================================================\033[0m")
