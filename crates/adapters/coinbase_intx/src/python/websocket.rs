...
use ant_core::python::{IntoPyObjectantExt, to_pyruntime_err, to_pyvalue_err};
use ant_model::{
    data::BarType,
...
use crate::websocket::{CoinbaseIntxWebSocketClient, messages::AntWsMessage};
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            ...
            tokio::spawn(async move {
                ...
                while let Some(msg) = stream.next().await {
                    match msg {
                        AntWsMessage::Instrument(inst) => Python::with_gil(|py| {
                            let py_obj = instrument_any_to_pyobject(py, inst)
                                .expect("Failed to create instrument");
                            call_python(py, &callback, py_obj);
                        }),
                        AntWsMessage::Data(data) => Python::with_gil(|py| {
                            let py_obj = data_to_pycapsule(py, data);
                            call_python(py, &callback, py_obj);
                        }),
                        AntWsMessage::DataVec(data_vec) => Python::with_gil(|py| {
                            for data in data_vec {
                                let py_obj = data_to_pycapsule(py, data);
                                call_python(py, &callback, py_obj);
                            }
                        }),
                        AntWsMessage::Deltas(deltas) => Python::with_gil(|py| {
                            call_python(py, &callback, deltas.into_py_any_unwrap(py));
                        }),
                        AntWsMessage::MarkPrice(mark_price) => Python::with_gil(|py| {
                            call_python(py, &callback, mark_price.into_py_any_unwrap(py));
                        }),
                        AntWsMessage::IndexPrice(index_price) => Python::with_gil(|py| {
                            call_python(py, &callback, index_price.into_py_any_unwrap(py));
                        }),
                        AntWsMessage::MarkAndIndex((mark_price, index_price)) => {
                            Python::with_gil(|py| {
                                call_python(py, &callback, mark_price.into_py_any_unwrap(py));
                                call_python(py, &callback, index_price.into_py_any_unwrap(py));
                            });
                        }
                        AntWsMessage::OrderEvent(msg) => Python::with_gil(|py| {
                            let py_obj =
