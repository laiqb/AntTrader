...
use ant_core::{datetime::NANOSECONDS_IN_MILLISECOND, nanos::UnixNanos};
use ant_model::{
...
/// Parses a ant instrument ID from the given Coinbase `symbol` value.
#[must_use]
...
pub const fn parse_aggressor_side(side: &Option<CoinbaseIntxSide>) -> AggressorSide {
    match side {
        Some(CoinbaseIntxSide::Buy) => ant_model::enums::AggressorSide::Buyer,
        Some(CoinbaseIntxSide::Sell) => ant_model::enums::AggressorSide::Seller,
        None => ant_model::enums::AggressorSide::NoAggressor,
    }
}
...
pub const fn parse_execution_type(liquidity: &Option<CoinbaseIntxExecType>) -> LiquiditySide {
    match liquidity {
        Some(CoinbaseIntxExecType::Maker) => ant_model::enums::LiquiditySide::Maker,
        Some(CoinbaseIntxExecType::Taker) => ant_model::enums::LiquiditySide::Taker,
        _ => ant_model::enums::LiquiditySide::NoLiquiditySide,
    }
}
