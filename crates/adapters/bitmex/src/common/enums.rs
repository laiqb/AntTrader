...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "ant_trader.core.ant_pyo3.bitmex", eq, eq_int)
)]
...
impl BitmexSide {
    /// Try to convert from ant OrderSide.
    ///
    /// # Errors
    ///
...
impl BitmexOrderType {
    /// Try to convert from ant OrderType with anyhow::Result.
    ///
    /// # Errors
    ///
...
impl BitmexTimeInForce {
    /// Try to convert from ant TimeInForce with anyhow::Result.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn test_time_in_force_conversions() {
        use ant_model::enums::TimeInForce;

        // BitMEX to ant (all supported variants)
        assert_eq!(
        ...
        // ant to BitMEX (all supported variants)
        assert_eq!(
