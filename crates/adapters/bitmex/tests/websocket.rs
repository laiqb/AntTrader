...
use ant_bitmex::websocket::client::BitmexWebSocketClient;
use ant_model::identifiers::AccountId;
use rstest::rstest;
use serde_json::json;
...
async fn test_subscribe_to_public_data() {
    ...
    // Subscribe to trades
    let instrument_id = ant_model::identifiers::InstrumentId::from("XBTUSD.BITMEX");
...
async fn test_multiple_symbols_subscription() {
    ...
    let xbt_id = ant_model::identifiers::InstrumentId::from("XBTUSD.BITMEX");
    let eth_id = ant_model::identifiers::InstrumentId::from("ETHUSD.BITMEX");
